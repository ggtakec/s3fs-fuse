...
//
int FdEntity::PreMultipartUploadRequest(PseudoFdInfo* pseudo_obj, bool is_copy)
{
    if(!pseudo_obj){
    ...
    std::string upload_id;
    int         result;
    if(0 != (result = pre_multipart_upload_request(path, orgmeta, false, upload_id))){
        return result;
    }
...
int FdEntity::NoCachePreMultipartUploadRequest(PseudoFdInfo* pseudo_obj)
{
    ...
    int result;
    if(0 != (result = PreMultipartUploadRequest(pseudo_obj, false))){
        return result;
    }
...
int FdEntity::RowFlushStreamMultipart(PseudoFdInfo* pseudo_obj, const char* tpath)
{
    ...
    if(pagelist.Size() <= S3fsCurl::GetMultipartSize()){
        ...
        if(!pseudo_obj->IsUploading()){
            ...
            //
            if(0 != (result = PreMultipartUploadRequest(pseudo_obj, true))){
                return result;
            }
        }

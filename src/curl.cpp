...
std::unique_ptr<S3fsCurl> S3fsCurl::CopyMultipartPostRetryCallback(S3fsCurl* s3fscurl)
{
    ...
    // setup new curl object
    if(0 != newcurl->CopyMultipartSetup(s3fscurl->b_from.c_str(), s3fscurl->path.c_str(), part_num, upload_id, s3fscurl->b_meta)){
        S3FS_PRN_ERR("Could not duplicate curl object(%s:%d).", s3fscurl->path.c_str(), part_num);
        return nullptr;
    }
    return newcurl;
...
std::unique_ptr<S3fsCurl> S3fsCurl::CreateParallelS3fsCurl(const char* tpath, int fd, off_t start, off_t size, int part_num, bool is_copy, etagpair* petag, const std::string& upload_id, int& result)
{
    ...
    if(!is_copy){
        ...
        if(0 != (result = s3fscurl->CopyMultipartSetup(tpath, tpath, part_num, upload_id, meta))){
            S3FS_PRN_ERR("failed uploading part setup(%d)", result);
            return nullptr;
        }
    }
    ...
    for(fdpage_list_t::const_iterator iter = mixuppages.begin(); iter != mixuppages.end(); ++iter){
        if(iter->modified){
            ...
            for(off_t i = 0, bytes = 0; i < iter->bytes; i += bytes){
                ...
                // initiate upload part for parallel
                if(0 != (result = s3fscurl_para->CopyMultipartSetup(tpath, tpath, s3fscurl_para->partdata.get_part_number(), upload_id, meta))){
                    S3FS_PRN_ERR("failed uploading part setup(%d)", result);
                    return result;
                }
...
int S3fsCurl::CopyMultipartSetup(const char* from, const char* to, int part_num, const std::string& upload_id, const headers_t& meta)
{
...
int S3fsCurl::MultipartPutHeadRequest(const std::string& from, const std::string& to, int part_number, const std::string& upload_id, const headers_t& meta)
{
    ...
    // setup
    if(0 != (result = CopyMultipartSetup(from.c_str(), to.c_str(), part_number, upload_id, meta))){

...
{
    pre_multipart_upload_req_thparam* pthparam = static_cast<pre_multipart_upload_req_thparam*>(arg);
    if(!pthparam){
        return reinterpret_cast<void*>(-EIO);
    }
    S3FS_PRN_INFO3("Pre Multipart Upload Request [path=%s][meta count=%lu][is copy=%s]", pthparam->path.c_str(), pthparam->meta.size(), (pthparam->is_copy ? "true" : "false"));

    S3fsCurl s3fscurl(true);
    pthparam->result = s3fscurl.PreMultipartUploadRequest(pthparam->path.c_str(), pthparam->meta, pthparam->upload_id, pthparam->is_copy);
...
{
    complete_multipart_upload_req_thparam* pthparam = static_cast<complete_multipart_upload_req_thparam*>(arg);
    if(!pthparam){
        return reinterpret_cast<void*>(-EIO);
    }
...
int multipart_put_head_request(const std::string& strfrom, const std::string& strto, off_t size, const headers_t& meta)
{
    ...
    // get upload_id
    if(0 != (result = pre_multipart_upload_request(strto, meta, true, upload_id))){     // always is_copy is true
        return result;
    }

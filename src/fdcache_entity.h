...
class FdEntity
{
    ...
    private:
        ...
        int GetObjectRequest(int fd, off_t start, off_t size) const REQUIRES(FdEntity::fdent_lock);
        int PreMultipartUploadRequest(PseudoFdInfo* pseudo_obj, bool is_copy) REQUIRES(FdEntity::fdent_lock, fdent_data_lock);
        int NoCachePreMultipartUploadRequest(PseudoFdInfo* pseudo_obj) REQUIRES(FdEntity::fdent_lock, fdent_data_lock);
